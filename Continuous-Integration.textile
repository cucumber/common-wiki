...
bq. Example. To get started using cucumber in "Hudson":http://hudson.dev.java.net, just add a build step running '<code>cucumber -f junit --out %WORKSPACE%</code>' and then check the 'Publish JUnit test result report', and enter <code>*.xml</code> in the 'Test report XMLs' field. Save and run.  (Later improve by using a rake task and placing your output in a subfolder).
