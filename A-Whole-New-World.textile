...
Now you can call <code>a_helper</code> from any step you want. Note that every scenario is run in a separate instance of the world, so there is no implicit state-sharing from scenario to scenario.

Once you have defined a World class you can extend it with other modules to add additional helper methods. In *version 0.2.3.2 and later* you can simply pass in a module to the World method to extend it.  For example:
