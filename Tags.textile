...
Tags which are comma separated are ANDed:

Example:  Running scenarios which match both @important AND @billing

<pre><code>cucumber --tags @billing,@important</code></pre>

Tags which are passed in separate --tags are ORed

Example:  Running scenarios which match @important OR @billing

<pre><code>cucumber --tags @billing --tags @important</code></pre>

You can combine these two methods to create powerful selection criterias:

Example: Running scenarios which match: (@billing AND @WIP) OR @important
