...
h2. Running a subset of scenarios
...
<pre><code>cucumber --tags ~@todo --tags ~@wip</code></pre>
...
h2(#overriding_tags_from_profile). Overriding the tag filters from a profile

It is currently not possible to override the tag filters from a profile.

The default profile, for example, includes a ==<code>--tags ~@wip</code>== filter. But what if you want to use everything from the default profile __except__ the `--tags ~@wip` portion?
...
But because that is effectively doing an "and" between ==<code>--tags ~@wip</code>== and ==<code>--tags @wip,~@wip</code>==, it does match any scenarios.
...
*@ignore*: Ensures the scenario is not executed. <code>@CucumberOptions</code> need to includes this tag filter : <code>tags = {"~@ignore"}</code>