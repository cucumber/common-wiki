...
Cucumber supports tables in feature files.  These are roughly analogous to "Framework for Integrated Test":http://fit.c2.com/ (FIT) tables.  You can use these when you are specifying behaviour that changes at some data threshold.  I tend to avoid them in feature files and use them sparingly in step definition files.  I do not have any explicit reason for this avoidance but, using tables in feature statements just does not seem to go together in my head. 
