...
If you do not know what webrat is and you are doing web application testing then you should "read about it":http://agilewebdevelopment.com/plugins/webrat.
...
Realize that tests/assertions/expectations either pass or fail and that fail is NOT the same as false, whereas anything but fail is a pass.  When, in RSpec, <code>something.should_be 0</code> and it is not, then what is returned is *fail* and not *false*.  In raw cucumber (pardon the pun) one writes <code>fail if false</code> and not simply <code>false</code>.  A little reflection reveals why this is so, since false might be the expected successful outcome of a test.  However, the distinction between fail and false escaped my notice until I was confronted with it in an actual test suite.
