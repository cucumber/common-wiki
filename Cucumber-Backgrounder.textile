...
But, this will throw a method missinf exception:
<pre>
And /some other act.*/ do 
</pre>

Because *_And_* is not defined as a steps definition method.  Because the step definition match depends only upon the pattern, and not upon the step method, I have adopted the practice of only using <code>When /I have a match/ do</code> in my step definitions files.
Which raises another interesting point.  If more than one of these matchers are defined anywhere in any of your step_definitions.rb files, and not necessarily in the same file mind you, then cucumber would tell you that it found multiple step definition matches for a single feature step and force you to distinguish them.
