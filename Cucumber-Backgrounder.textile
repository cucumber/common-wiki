...
The foregoing give a sample of the form that feature statements take.  These statements, called <em>feature steps</em>, make up the user interface to Cucumber testing.  They are written in the "Imperative Style":http://www.benmabey.com/2008/05/19/imperative-vs-declarative-scenarios-in-user-stories/ for illustrative purposes, in practice all those Then/Ands would be subsumed into the simple Declarive statement <code>I should create the Cucumber environment</code>. We will return to that topic later.  For the moment we deal with the logical arrangement of Cucumber files within the context of a RoR project.
