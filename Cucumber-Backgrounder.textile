...
Finally, running <code>autotest</code> with the environment variable <code>AUTOFEATURE=true</code> will run ALL tests, including those in <code>/test</code>.  It will also load all test fixtures and this may leave the database in an indefinite state when cucumber features are run.  It is wise therefore, to write cucumber steps so that they do not depend upon an empty database or themselves place the database in the requisite state.
