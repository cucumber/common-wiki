...
The significant things here are that the method (Given) takes as its argument a "regexp":http://en.wikipedia.org/wiki/Regexp bounded by  <code>/</code> ( <em>although a quoted "simple string" can also be used</em> ) and that the matcher method is followed by a block.  In other words, written defferently this matcher method would look like this:

<pre><code>Given( /there are (\d+) froobles/ ) { |n|
.  .  .
}</code></pre>

Among other things, this means that step defintion method blocks receive all of the matcher arguments as string values.  Thus <code>n.to_i.times</code> and not simply <code>n.times</code>.  It also means that step matchers themselves can be followed by the special regexp modifiers, like <strong>i</strong> if you want to avoid issues involving capitalization.
