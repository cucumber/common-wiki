...
h3. Note Respecting cucumber-rails v0.4.1

It seems that the latest versions of cucumber and cucumber-rails are oriented towards Ruby-1.9.2.  If you are using Ruby-1.8.7 and employing testunit and Capybara then the following must be added to the <code>support/env.rb</code> file produced by the rails generator:
<pre>
# Nonetheless, these changes are required
ENV["RAILS_ENV"] = "test"
if RUBY_VERSION =~ /1.8/
...
This may no longer be case following the release of cucumber-rails v0.5.0 presently in beta.
