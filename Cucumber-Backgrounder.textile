...
Depending upon the version of Rails, running <code>rails g cucumber</code> or <code>script/generate cucumber</code> adds this layout to the existing structure:
<pre><code>
|-- features
|   |-- step_definitions
`   |   `-- web_steps.rb
...
<em>If you are not using Cucumber-Rails then you can accomplish much the same thing by creating the directory tree from the command line (mkdir -p features/step_definitions) and simply ignoring the support directory and all of the (Rails specific) files</em> .  Once the features directory structure is in place then we are ready to begin testing with Cucumber.
...
Sticking with this approach you can face significant design changes ( <em>and gem updates</em> ) with complete equanimity, secure in the knowledge that if unanticipated changes break anything anywhere in your project then you will know of this immediately upon running your test suite.  More importantly, you will know exactly what is broken and where it is broken.  Like any skilled tradesman, most of the value programmers provide lies in discovering what piece of code to change and not simply how to change it.  Finding the the spots that need attention in a application is usually the biggest maintenance problem.  Strictly coding to features simplifies that task to the point of triviality.
