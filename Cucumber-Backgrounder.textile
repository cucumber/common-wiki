...
Cucumber-Rails is pre-configured with support for view integration testing using either "Capybara":http://github.com/jnicklas/capybara (script/generate  cucumber --capybara) or "Webrat":http://github.com/brynary/webrat (script/generate cucumber).  If you do not know what Capybara or Webrat are or do,  and you are doing web application testing, then you should find out more about both.  Capybara is emerging as the preferred candidate, however it does not play well with Rails 2.x builtin Test Assertions.  In particular the response.body method seems to have disappeared.  

The Cucumber-Rails generator will set up the necessary support files for whichever you choose.
