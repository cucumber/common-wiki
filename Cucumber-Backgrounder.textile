...
As is the case for many ( <em>most? all?</em> ) programming suggestions there are exceptions and contrary opinions respecting the orthodox advice respecting mating feature files and step definition files.  Nonetheless, it is probably best to follow this recommendation at the outset and depart from it only when you are experienced enough to evaluate the trade-offs between approaches.
...
What is important about the difference in styles?  The first example, <strong>When "Bob" logs in</strong>,  is a functional requirement.  The second, much longer, example is a procedural reference.  Procedures belong in the implementation details.  "<em>Note that <strong>Plain English</strong> folks advocate the use of must in place of should or shall.</em>":http://www.plainlanguage.gov/whatisPL/definitions/Kimble.cfm
