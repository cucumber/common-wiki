...
Cucumber provides a simple method to organize features and scenarios by user determined classifications.  This is implemented using the convention that any string found in a feature file that is prefaced with the commercial at (<strong>&#64;</strong>) symbol is considered a tag.  As distributed, Cucumber-Rails builds a Rake task that recognizes the <em>&#64;wip</em> tag.  However, any string may used as a tag and any scenario of feature can have multiple tags associated with it.  For example:
...
Given that the forgoing is contained in a file called <code>features/login.feature</code> and that the cucumber-rails gem is installed and configured then you can exercise this scenario in any of the following ways:
...
Tags may also be negated by prefacing the tag with the tilde (<strong>~</strong>) symbol.  In other words, you can exclude all scenarios that have a particular tag.  For example, the following not exercise any scenario having the tag <em>&#64;ignore</em> found in the directory tree rooted at <code>features/wip</code>:

<pre>
$ cucumber --require=features --tag=~@gnore features/wip
</pre>
