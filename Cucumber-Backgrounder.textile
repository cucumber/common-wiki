...
As is the case for many ( <em>most? all?</em> ) programming suggestions there are exceptions and contrary opinions respecting the orthodox position on the practice of mating step definition files to specific feature files.  Nonetheless, it is probably best to follow the recommendation to avoid feature-specific step-definition files at the outset.  Thereafter depart from it only when you are experienced enough to evaluate the trade-offs between approaches.
