...
Note that you are not required to use a matched subpattern (.*) in the following block.  It is considered better form by some to surround with double quotation marks, <strong>"</strong>, elements in the feature step clauses that meant to be taken as values. This is only a convention, however, if you choose to follow this road then you must adjust your step files accordingly.  For example:
...
And /an invoiced non-shipped situation/ do
