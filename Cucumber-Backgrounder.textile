...
Since version 0.6.0 one can no longer overcome this default setting by adding the <code>--tags=@wip</code> to the Cucumber argument list on the command line as each separate <code>--tags</options> are ANDed together.  Thus the combination of --tags @wip AND --tags ~@wip fails everywhere.  You must create a special profile in <code>config/cucumber.yml</code> to deal with this or alter the default profile to suit your needs.  Note as well that @wip tags are a special case. If any scenario tagged as @wip passes all of its steps without error then Cucumber reports this as Scenario as failing since a work in progress is not supposed to pass.
