...
Realize that tests either pass or fail and that fail is NOT the same as false.  When, in RSpec, <code>something.should_be 0</code> and it is not, then what is returned is *fail* and not *false*.  In raw cucumber (pardon the pun) one writes <code>fail if false</fail> and not simply <code>false</code>.
