...
<em>If you are not using Cucumber-Rails then you can accomplish much the same thing by manipulating the file system directly from the command line ( mkdir -p features/support etc. ) and ignoring the files</em> .  Once the features directory structure is in place then we are ready to begin testing with Cucumber.
