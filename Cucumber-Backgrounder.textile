...
Since the original version of this article appeared, Cucumber has undergone repeated revisions and refactorings.  Among these was the sensible decision to move framework specific implementations into their own gems.  Consequently, installing Cucumber for a framework now frequently starts with installing the specific framework cucumber gem, which will pull in the core cucumber gem as a dependency.
...
Then you will create a sub-directory named "features"
  ...
  And you will created an environment cucumber.rb in config/environments
  And you will modify config/database.yml
  .  .  .
</pre>
...
If more than one matcher in all of the step definitions files matches a feature step then cucumber complains that it found multiple step definition matches for that step and forces you to distinguish them.  You can force cucumber to make a decision instead by passing the switch <code>--guess</em> on the command line.
...
If all your feature's scenarios share the same 'setup' feature steps then Cucumber provides the <em>Background</em> section.  Steps contained within a Bacground section are run before each of the scenarios.
...
Cucumber-Rails preconfigured with support for integration testing using either "Capybara":http://github.com/jnicklas/capybara (script/generate  cucumber --capybara) or Webrat (script/generate cucumber), the default.  If you do not know what Capybara or Webrat are or do,  and you are doing web application testing, then you should find out more about "Capybara":http://github.com/jnicklas/capybara and "Webrat":http://github.com/brynary/webrat, both.

The cucumber generater will set up the necessary support files whichever you choose.
