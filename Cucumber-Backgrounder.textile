...
When encountered these nested Then and When steps should be replaced with the step method:
...
My rule of thumb is that if a step definition is called from more than one other step definition then its contents probably should be extracted out into a custom method.  For example our logon step is likely to be used repeatedly throughout our features.  Turning it into a method is probably called for.
<pre><code>
