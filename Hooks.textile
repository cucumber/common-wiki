...
<code>After</code> hooks will be run after the last step of each scenario. They will run in the *opposite* order of which they are registered.
...
Sometimes you may want a certain before or after hook to run only for certain scenarios. This can be achieved by associating a <code>Before</code>, <code>After</code> or <code>AfterStep</code> hook with one or more [[tags]]. Example:
