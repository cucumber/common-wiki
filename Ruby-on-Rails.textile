...
*important:* As of this writing, Cucumber 0.1.13  depends on Webrat code that isn't officially released as a gem on Rubyforge. In the meanwhile, install Aslak Hellesøy's snapshot gem:
...
<pre><code>
Given /logged in as "(.*)"/ do |login|
  @current_user = User.create!(
        :login=>login,   :password =>              'generic',
        :email=>"#{login}@example.com", :password_confirmation => 'generic'
        )
#  @current_user.activate! # :create syntax for restful_authentication w/ aasm. Tweak as needed.
