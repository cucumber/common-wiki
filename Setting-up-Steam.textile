Steam is a set of ruby bindings to Java's HtmlUnit which allows you to run webrat-style step definitions against code which relies on javascript to work. It is similar to celerity/culerity but doesn't require an extra process as it is able to run inline thanks to RJB.
...
To allow the webrat step definitions to function the same between webrat and steam you should replace cucumber's standard webrat_steps.rb file with one I've written to handle webrat and steam automatically: "webrat_compatible_steps.rb":http://github.com/svenfuchs/steam/blob/master/example/cucumber/step_definitions/webrat_compatible_steps.rb
