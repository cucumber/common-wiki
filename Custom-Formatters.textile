...
In each case, the event object is passed into the block in `config.on_event`. Each event object belongs to an event class corresponding to the specific event in question, e.g., `Cucumber::Events::TestRunStarted`, `Cucumber::Events::GherkinSourceRead`, etc. Depending on the event, different objects are accessible through methods called on the event object. For instance, for the `Cucumber::Events::GherkinSourceRead` event, the Gherkin body is available using the `#body` method, and for the `Cucumber::Events::TestCaseStarted` event, the individual test cases (features) are available via the `#test_cases` method.
