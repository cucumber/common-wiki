...
*Second* - add <code>require 'cucumber/formatters/unicode'</code> to your <code>env.rb</code> file. 

*Third* - set your command window's font to a font that has the characters you're outputting - for example _Upper Left corner icon menu -> Properties -> Font -> Lucida Grande_.
...
h2. Changes made to models while running stories with Selenium are not visible from within step definitions code
...
This is happening if you have enabled <code>Cucumber::Rails.use_transactional_fixtures</code> option in your <code>env.rb</code> file. To solve this problem, you need to comment out this option and handle table cleanup yourself, for example using <code>Before</code> block. Please read [[Hooks]] to get an idea how to use it.