...
Options are declared in the config/[[cucumber.yml]] file (which must be present and have a default or autotest profile).  To change the way the features are run with autotest create two profiles in your [[cucumber.yml]]: 
