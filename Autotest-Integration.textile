...
* <code>autotest</code> : This profile is used when Autotest wants to run only the failing scenarios.
* <code>autotest-all</code> : This profile is used when Autotest wants to run all the scenarios (right after a red->green transiation).

Please see [[Running Features]] for more information about profiles.
