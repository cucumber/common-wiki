...
Also check out [[Multiline Step Arguments]] for more info on how to pass entire tables to your step definitions.

h2. Successful steps

When Cucumber finds a matching Step Definition it will execute it. If the block in the step definition doesn't raise an Error, the step is marked as successful (green). What you return from a Step Definition has no significance what so ever.
...
<pre><code>Given /we had (.*)/ do |what|
end

# A really versatile step!
...
<pre><code>Given we had drinks on the roof</code></pre>

The plain text step will match both step definitions and raise <code>Cucumber::Ambiguous</code>. However,
if you run Cucumber with <code>--guess</code>, it will guess that you were aiming for the versatile step.
...
# If there are 2+ definitions with the same amount of capture groups, the one with the shortest overall captured string length wins
# If there are still 2+ options the Ambiguous error is raised

So if you try <code>--guess</code> with the mice above, you'll still get <code>Cucumber::Ambiguous</code>. Still, using
guess mode is useful in some situations.
