...
As of RSpec 1.2.8 if you need to use stubs within Cucumber and you are using the built in mocking/stubbing framework that ships with RSpec you simply need to <pre>require 'spec/stubs/cucumber'</pre> in your env.rb file. Note that message expectations (should_receive) are _not_ automatically verified.
